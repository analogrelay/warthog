...
pub use self::types::{ValType, FuncType, TableType, MemoryType, GlobalType};
pub use self::section_header::{SectionId, SectionHeader};
pub use self::type_section::TypeSection;
pub use self::data_section::DataSection;
pub use self::member_desc::MemberDesc;
pub use self::import_section::{ImportSection, Import};
pub use self::export_section::{ExportSection, Export};
...
impl<R: io::Read> Reader<R> {
    ...
    pub fn read_section_header(&mut self) -> Result<Option<SectionHeader>, Error> {
        ...
        Ok(Some(SectionHeader{ id, size: utils::read_leb128_u32(&mut self.source)? }))
    }
